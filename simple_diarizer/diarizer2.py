...
class Diarizer:
    ...
    def windowed_embeds(self, signal, fs, windows=[1.5, 2], period=0.75):
        """
        ...
        with torch.no_grad():
            for i, j in segments:
                ...
                for idx, w in enumerate(windows):
                    ...
                    seg_embeds.append(seg_embed*self.weights[idx])
                seg_embeds = torch.stack(seg_embeds)
                seg_embed = torch.sum(seg_embeds, dim = 0)
    ...
    def recording_embeds(self, signal, fs, speech_ts):
        ...
        for utt in tqdm(speech_ts, desc="Utterances", position=0):
            ...
            utt_embeds, utt_segments = self.windowed_embeds(
                utt_signal, fs, self.windows, self.period
            )
